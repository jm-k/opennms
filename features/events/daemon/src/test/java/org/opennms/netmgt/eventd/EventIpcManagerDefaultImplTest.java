...
public class EventIpcManagerDefaultImplTest extends TestCase {
    ...
    public void testDiscardWhenFullWithSlowEventHandler() throws InterruptedException {
        ...
        manager.setHandlerDiscardWhenFull(true);
        manager.afterPropertiesSet();
    ...
    public void testDoNotDiscardWhenFullWithSlowEventHandler() throws InterruptedException {
        AtomicInteger counter = new AtomicInteger();
        ...
        manager.setHandlerPoolSize(1);
        manager.setHandlerQueueLength(5);
        manager.setHandlerDiscardWhenFull(false);
        manager.afterPropertiesSet();

        // Send 10 events. The first one will be executed on the handler thread,
        // the next 5 will be enqueued, then the execution will switch between the
        // handler pool and the caller thread until all of the events are processed.
        //
        for (int i = 0; i < 10; i++) {
            EventBuilder bldr = new EventBuilder("uei.opennms.org/foo/" + i, "testDiscardWhenFullWithSlowEventListener");
            Event e = bldr.getEvent();
    ...
    public void testDiscardWhenFullWithSlowEventListener() throws InterruptedException {
        AtomicInteger counter = new AtomicInteger();
    ...
    public void testDoNotDiscardWhenFullWithSlowEventListener() throws InterruptedException {
        AtomicInteger counter = new AtomicInteger();
        ...
        // Send 10 events. The first one will be executed on the listener thread,
        // the next 5 will be enqueued, then the execution will switch between the
        // listener thread and the caller thread until all of the events are processed.
        //
        for (int i = 0; i < 10; i++) {
            EventBuilder bldr = new EventBuilder("uei.opennms.org/foo/" + i, "testDiscardWhenFullWithSlowEventListener");
            Event e = bldr.getEvent();
